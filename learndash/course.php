...
<?php if ( $course_status != __( 'Completed', 'learndash' )  || learndash_is_user_in_group($user_id, 1974)) :
	// echo $content; 
else : ?>
<!-- Include whatever you want to display instead of the course content. I chose to display the message below, plus intuitive navigation to additional courses that are available. You could just as easily provide a link to download their certificate or show a list of courses they've completed etc. -->
	...
	<p>Ready for your next course? Browse our extensive range of courses!</p>
	<br />
...
<?php if ( $logged_in ) : ?>
	...
	<?php  if ( ! empty( $course_certficate_link ) ) : ?>
		<div id="learndash_course_certificate">
			<a href='<?php echo esc_attr( $course_certficate_link ); ?>' class="btn btn-success btn-large" target="_blank"><?php echo apply_filters('ld_certificate_link_label', __( 'Print Your Certificate', 'learndash' ), $user_id, $post->ID ); ?></a>
		</div>
		<br />
	<?php endif; ?>
<?php endif; ?>
...
<?php if ( isset( $materials ) && $course_status != __( 'Completed', 'learndash' )  ) : ?> <!-- Checks if Course is Completed -->
...
<?php if ( $has_course_content && $course_status != __( 'Completed', 'learndash' )  ) : ?> <!-- Checks if Course is Completed -->
	<?php 
